...
class Upload_company_via_csv extends MY_Controller {
    ...
    public function ipp()
    {
    $query = $this->db->query("SELECT * FROM companies WHERE created_at >= '2016-03-08'  LIMIT 10");
       
          foreach ($query->result_array() as $row)
          {
              //echo $row['registration'].' ' .$row['id'] .'<br>';
              $response  = $this->getCompanyHouseChargesApi($row['registration'],$row['id']);
              
                  if($response[0]['company_id']) $output[] = $response[0] ;
        ...
        $this->db->insert('mortgages', $output[0]); 
        print_r($output[0]);
        // echo  $query->num_rows();
